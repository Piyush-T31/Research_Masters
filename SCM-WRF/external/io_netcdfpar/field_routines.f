...
subroutine ext_ncdpar_RealFieldIO(IO,NCID,VarID,VStart,VCount,Data,Status)
  ...
  if(Status /= WRF_NO_ERR) then
    ...
    write(0,*) 'vstart = ',vstart
    write(0,*) 'vcount = ',vcount
    write(msg,*) 'NetCDF error in ',"field_routines.F90",', line', 77
...
subroutine ext_ncdpar_DoubleFieldIO(IO,NCID,VarID,VStart,VCount,Data,Status)
  ...
  call netcdf_err(stat,Status)
  if(Status /= WRF_NO_ERR) then
    write(msg,*) 'NetCDF error in ',"field_routines.F90",', line', 109
    call wrf_debug ( WARN , msg)
  endif
  return
...
subroutine ext_ncdpar_IntFieldIO(IO,NCID,VarID,VStart,VCount,Data,Status)
  ...
  call netcdf_err(stat,Status)
  if(Status /= WRF_NO_ERR) then
    write(msg,*) 'NetCDF error in ',"field_routines.F90",', line', 138
    call wrf_debug ( WARN , msg)
  endif
  return
...
subroutine ext_ncdpar_LogicalFieldIO(IO,NCID,VarID,VStart,VCount,Data,Status)
  ...
  if(stat/= 0) then
    ...
    write(msg,*) 'Fatal ALLOCATION ERROR in ',"field_routines.F90",', line', 164
  ...
  call netcdf_err(stat,Status)
  if(Status /= WRF_NO_ERR) then
    write(msg,*) 'NetCDF error in ',"field_routines.F90",', line', 188
    call wrf_debug ( WARN , msg)
    return
  endif
  ...
  if(stat/= 0) then
    ...
    write(msg,*) 'Fatal DEALLOCATION ERROR in ',"field_routines.F90",', line', 195
    call wrf_debug ( FATAL , msg)
    return
  endif
  return
