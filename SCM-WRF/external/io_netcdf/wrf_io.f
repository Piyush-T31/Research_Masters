...
!    write(unit=0, fmt='(2x, 3a,i6)')  'file: ', "wrf_io.F90", ', line: ', 2667
...
!    write(unit=0, fmt='(10x, a,i6)')    'compression_level = ', compression_level
     ...
     call netcdf_err(stat,Status)
     if(Status /= WRF_NO_ERR) then
       write(msg,*) 'ext_ncd_write_field: NetCDF def chunking error for ',TRIM(VarName),' in ',"wrf_io.F90",', line', 2677
       call wrf_debug ( WARN , TRIM(msg))
       return
     endif
      ...
      call netcdf_err(stat,Status)
      if(Status /= WRF_NO_ERR) then
         write(msg,*) 'ext_ncd_write_field: NetCDF def compression  error for ',TRIM(VarName),' in ',"wrf_io.F90",', line', 2685
         call wrf_debug ( WARN , TRIM(msg))
         return
      endif
  endif
